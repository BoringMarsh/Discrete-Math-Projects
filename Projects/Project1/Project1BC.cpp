...
Map_ic getProposition(string formula)
{
	...
	for (unsigned int i = 0; i < formula.length(); i++)
	{
		...
		else if (!priority.count(c))  //出现非法字符则直接报错退出
		{
...
{
	int p, q, result;
	char opt = opter.top();  //取栈顶的运算符，根据运算需要取1或两个操作数，运算完成后结果放回栈中。栈顶运算符出栈
	...
	case ')':
		opter.pop();
		while (opter.top() != '(')  //继续运算直到下一个左括号出现
		{
			check(value, opter);
		}
		if (opter.top() == '(')  //将下一个左括号压入栈中
		{
			opter.pop();
		}
		break;
...
int calculate(string formula, Map_ic pSet, Map_ii value)
{
	...
	for (unsigned int i = 0; i < formula.length(); i++)
	{
		...
		if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) 
		{
			pvalue.push(value[findProposition(pSet, c)]);  //将某个命题变项的取值找到，放入栈中
		}
		else 
		{
			//此时遍历的是运算符
			char tmp = opter.top();  //取栈顶运算符
			if (priority[tmp] > priority[c])  //若栈顶优先级高，则运算到栈顶优先级低于c或者有括号出现
			{
				while (priority[tmp] > priority[c] && tmp != '(')
				{
					check(pvalue, opter);
					tmp = opter.top();
					if (tmp == '#' && c == '#')  //若遇到#，则说明运算完成，返回结果
					{
						return pvalue.top();
					}
				}
				opter.push(c);  //栈顶优先级低于c，c进栈
			}
			else
				opter.push(c);  //栈顶优先级低于c，c进栈
		}
...
{
	string::const_iterator it = str.begin();  /*游标对象*/
	stack<char> brackets;  /*记录括号情况的栈*/
	int op1Num = 0, op2Num = 0, numNum = 0;  /*操作符（单目、双目）个数，数字个数*/
	...
	while (it != str.end()) {
		...
		if (*it == '(')  /*若遇到左括号，压入栈中*/
			brackets.push(*it);
		else if (*it == ')') {  /*若遇到右括号，栈不为空则弹出一个元素；栈为空则说明右括号多余，非法*/
			if (!brackets.empty())
				brackets.pop();
	...
	if (!brackets.empty()) {  /*若扫描完成栈仍不为空，则左括号多余，非法*/
		cout << "括号不匹配" << endl;
		return false;
	}

	if (numNum != op2Num + 1) {  /*若双目运算符个数不等于数字个数+1，则不匹配，非法*/
...
int main() 
{
	...
	if (!formulaCheck(formula))  //检查公式是否合法
		return 0;

	Map_ic proposition_set = getProposition(formula);  //统计命题变项
	cout << "该式子中的变量个数为：" << proposition_set.size() << endl << "输出真值表如下：" << endl;
	for (unsigned int i = 0; i < proposition_set.size(); i++)
	{
		cout << proposition_set[i] << "\t";  //输出所有的命题变项
	}
	cout << formula << endl;
	int *m;
	m = (int*)malloc(sizeof(int)*pow2(proposition_set.size()));   //该数组依次存放命题公式的各行(0或1)的运算结果的值
	for (int i = 0; i < pow2(proposition_set.size()); i++)  //有2^n种取值，遍历一遍
	{
		Map_ii bina_set = toBinary(proposition_set.size(), i);  //将取值转换为二进制
		for (unsigned int j = 0; j < bina_set.size(); j++)
		{
			cout << bina_set[j] << "\t";  //将各命题变项的值打印出来
		}
		int result = calculate(formula, proposition_set, bina_set);  //计算公式真值
		*(m+i) = result;  //记录这一趟的真值
		cout << result << endl;  //打印这一趟的计算结果
	}
	int n_m = 0, n_M = 0;
	cout << "该命题公式的主析取范式：" << endl;
	for (int i = 0; i < pow2(proposition_set.size()); i++)  //统计成真赋值并打印主析取范式
	{
		if (*(m + i) == 1)
		{
			if (n_m == 0) 
	...
	for (int i = 0; i < pow2(proposition_set.size()); i++)   //统计成假赋值并打印主合取范式
	{
		if (*(m + i) == 0)
		{
			if (n_M == 0) 
